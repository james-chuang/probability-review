...
## **variance and covariance (DL 3.8, CS229 2.5)**
...
$$
Using the properties of the expected value (namely that expectations are linear and that $\mathbb{E}[X]$ is a constant wrt an outer expectation), we can derive an alternate expression for the variance:
$$
\begin{align}
\text{Var}[X] & = \mathbb{E}[(X-E[X])^2] \\
              & = \mathbb{E}[X^2-2X \mathbb{E}[X] + \mathbb{E}^2[X]] \\
              & = \mathbb{E}[X^2] - 2 \mathbb{E}\left[X \mathbb{E}[X]\right] + \mathbb{E}\left[\mathbb{E}^2[X]\right] \\
